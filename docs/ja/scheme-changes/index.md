---
title: "スキーム変更履歴 | pacs008"
description: "メッセージが受理されるかを左右するルール変更を、発効日ごとにまとめています。"
lang: ja-JP
layout: page
date: "2026-07-28"
lastUpdated: true
image: /logo.webp
canonical: /ja/scheme-changes/
robots: "index, follow"
draft: false
noindex: false
---
...
メッセージが受理されるかを左右するルール変更を、発効日ごとにまとめています。

ルールレジストリから生成、ルールセット `2026.11.0` (hash `sha256:a27fe2e7a04f36e9645310984e7142e58d2d5467490207479e689dd6fbbb668f`).

Swift は 2026年11月から年次の Standards Release サイクルへ移行するため、この一覧は期日で終わらず毎年増えていきます。

購読する: [Atom feed](/scheme-changes.xml).

## ルールセットのバージョン管理

ルール識別子はマイナーリリース間で安定です。合否の挙動が変わる場合は新しいルールセット版が必要で、これにより過去の報告を再現できます。
...
検証レポートにはルールセットの版とハッシュが記録されます。相違を報告する際は両方を示してください。判定を生んだルールセットを正確に再構成できます。
