...
|---|---|---|
| pacs.003.001.09 | pacs008의 현재 구현 | 현재 프로젝트에서 자동이체 참조를 모델링할 때 유용합니다. |
| pacs.003.001.10-11 | 이후 카탈로그 개정판 | 신규 구축에 사용하기 전에 이후 개정판에서 위임, 상태, 상호운용성 관련 업데이트를 확인하세요. |
...
| [`pacs.007.001.11`](/ko/pacs.007.001.11/) | 금융기관 간 지급 취소 | pacs.007 메시지는 아직 결제되지 않은 이전에 전송된 결제 지시를 취소하거나 결제된 결제의 취소를 요청하는 데 사용됩니다. pacs.004(반환)와 달리 원래의 지시 에이전트가 개시합니다. |
