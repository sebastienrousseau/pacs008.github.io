---
title: "2026 年 ISO 20022 就绪 | pacs008"
...
距 **2026 年 11 月 14 日** 还有 **108 天**（截至 2026-07-29）。

自该日起，完全非结构化的邮政地址将不再被 SWIFT CBPR+ 支付报文以及英格兰银行的 CHAPS 校验库接受。同一天还有另外两项变更生效，2027 年 11 月将有更大范围的变更。

下列每条规则都带有标识符、生效日期、权威来源和测试文件——无需仅凭信任。

## 这是否影响到您

如果您发送的 CBPR+ 或 CHAPS 支付中包含任一当事方的邮政地址，即属适用范围。

| | |
|---|---|
| **报文** | `pacs.008`, `pacs.009`, `pacs.004`, `pacs.003` |
| **当事方** | Debtor, creditor, ultimate debtor, ultimate creditor, and agents that carry an address |
| **不在范围内** | `admi.024`, `camt.025`, `camt.052`, `camt.053`, `camt.054`, `camt.060` |
| **豁免** | Agents identified by BIC alone need no postal address (`CBPR-ADDR-005`) |

## 实际改变了什么

该要求是**下限而非上限**。这是该强制要求中最常被误读的部分。

| 格式 | `TwnNm` | `Ctry` | `AdrLine` | 2026 年 11 月 14 日之前 | 自该日起 |
|---|---|---|---|---|---|
| 完全结构化 | 有 | 有 | 无 | 接受 | 接受 |
| 混合 | 有 | 有 | 有 | 接受 | **接受** |
| 完全非结构化 | 无 | 无 | 有 | 接受 | **拒绝** |

您**无需**把街道、门牌号和邮政编码迁入结构化元素。在 `<TwnNm>` 中填写城市、在 `<Ctry>` 中填写两位 ISO 3166 国家代码即可。其余内容可保留在地址行中：这属于混合地址，依然有效。

[Full detail, with worked examples →](/structured-address/)

## 立即检查您的数据

两个工具，均完全在您的浏览器中运行。不会发送任何支付数据。
...
2026 年 11 月并非 ISO 20022 变更的终点。自该日起 Swift 转为年度 Standards Release 周期，使用指南将逐年变化。
